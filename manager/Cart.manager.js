...
class CartManager {
  ...
  async saveData(doc) {
    ...
    await fsPromise.writeFile(this.filePath, docToString);
  }

  async readData() {
    try {
      const json = await fsPromise.readFile(this.filePath, "utf-8");
  ...
  async addProductToCart(cid, pid) {
    ...
    const productsPath = "./manager/data/products.json"; // Ajustar la ruta si es necesario
